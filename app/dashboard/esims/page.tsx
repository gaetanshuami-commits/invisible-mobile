      ...
      <div className="mx-auto max-w-5xl rounded-[32px] bg-white p-8 shadow-sm">
        ...
        {!esims || esims.length === 0 ? (
          ...
          <div className="mt-10 space-y-4">
            {esims.map((esim) => (
              <div key={esim.id} className="rounded-[28px] bg-[#f4f1e8] p-6">
                <p className="text-2xl font-semibold">{esim.country_code ?? "eSIM mondiale"}</p>
                <p className="mt-2 text-black/50">Statut : {esim.status}</p>
                <p className="mt-2 text-black/50">ICCID : {esim.iccid ?? "Non disponible"}</p>
              </div>
            ))}
          </div>
        )}
